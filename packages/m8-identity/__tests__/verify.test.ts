...
describe('verifyOffline', () => {
  ...
  it('enforces required claims', async () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.errors.some(e => e.includes('Missing required claims'))).toBe(
      true,
    )
    __clearTestPayload()
  })
  ...
  it('enforces minimum claims count', async () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.errors.some(e => e.includes('Insufficient claims'))).toBe(
      true,
    )
    __clearTestPayload()
  })
  ...
  it('marks revoked credential invalid', async () => {
    ...
    vi.mocked(fetch).mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          revoked: true,
          revokedAt: '2026-06-01',
          revokedBy: 'did:m8:ine:emisor-001',
        }),
      ),
    )
