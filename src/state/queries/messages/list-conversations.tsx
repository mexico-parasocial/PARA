          ...
          if (bsky.isType(chat.bsky.convo.defs.logBeginConvo, log)) {
            ...
            updateConvoInAllLists(
              ...
              withRevGuard(log.rev, convo => {
                ...
                // messageAndReactionView only carries a live MessageView, while
                // the log's message can also be a deleted/unknown view - only
                // refresh lastReaction when we have a usable message.
                const reactedMessage = bsky.isType(
                  chat.bsky.convo.defs.messageView,
                  log.message,
                )
                  ? log.message
