...
export function useLabelerInfoQuery({
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const res = await agent.appviewClient.call(app.bsky.labeler.getServices, {
  ...
  return useMutation({
    async mutationFn({did, subscribe}: {did: string; subscribe: boolean}) {
      ...
      if (labelerDids.length) {
        const profiles = await agent.appviewClient.call(
          app.bsky.actor.getProfiles,
          {actors: labelerDids},
        )
        for (const did of labelerDids) {
          const exists = profiles.profiles.find(p => p.did === did)
          if (exists) {
