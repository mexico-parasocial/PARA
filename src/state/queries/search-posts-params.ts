...
export type ExtractedSearchParams = {
  ...
  until?: string
  tag?: string[]
  /** PARA-specific: Mexican state filter */
  state?: string
  /** PARA-specific: district key filter */
  district?: string
  /** PARA-specific: cabildeo phase filter */
  phase?: string
  /** PARA-specific: political party filter */
  party?: string
  /** PARA-specific: community URI/handle filter */
  community?: string
  /** PARA-specific: cabildeo URI/handle filter */
  cabildeo?: string
  /** PARA-specific: political compass position filter */
  compass?: string
  /** PARA-specific: policy area tag filter (maps to tag) */
  area?: string[]
}
  ...
  for (const token of tokenizeQuery(query)) {
    ...
    switch (op) {
      ...
      case 'community':
        ...
        break
      case 'cabildeo':
        result.cabildeo ??= value
        break
      case 'compass':
        result.compass ??= value
        break
      case 'area':
        result.area ??= []
        result.area.push(value)
        break
      default:
...
/**
 * Merges two comma-separated value lists, dropping empties and duplicates while
 * preserving order.
 */
function mergeCsv(a?: string, b?: string): string | undefined {
...
/**
 * Merges two space-separated value lists, dropping empties and duplicates while
 * preserving order.
 */
...
/**
 * Converts operators lifted out of a free-text query into the string-based
 * SearchFilters representation used by the search screen. Scalar fields in the
 * lifted operators override existing filters; list fields are unioned. Used when
 * the user submits the search box directly so `state:Jalisco` becomes a
 * structured filter instead of only being parsed inside the advanced dialog.
 */
export function extractedOperatorsToSearchFilters(
  lifted: Omit<ExtractedSearchParams, 'q'>,
  baseFilters?: SearchFilters,
): SearchFilters {
  const filters: SearchFilters = {...baseFilters}

  // Upstream scalar operators - lifted value wins.
  if (lifted.author) filters.author = lifted.author
  if (lifted.mentions) filters.mentions = lifted.mentions
  if (lifted.domain) filters.domain = lifted.domain
  if (lifted.url) filters.url = lifted.url
  if (lifted.lang) filters.lang = lifted.lang
  if (lifted.since) filters.since = lifted.since
  if (lifted.until) filters.until = lifted.until

  // Upstream list operators - union with existing values.
  if (lifted.tag?.length) {
    filters.tag = mergeSpace(filters.tag, lifted.tag.join(' '))
  }

  // PARA scalar operators - lifted value wins.
  if (lifted.state) filters.state = lifted.state
  if (lifted.district) filters.districtKey = lifted.district
  if (lifted.phase) filters.cabildeoPhase = lifted.phase
  if (lifted.party) filters.party = lifted.party
  if (lifted.compass) filters.politicalCompassPositions = lifted.compass

  // PARA list operators - union with existing values.
  if (lifted.community) {
    filters.communityUris = mergeCsv(filters.communityUris, lifted.community)
  }
  if (lifted.cabildeo) {
    filters.cabildeoUris = mergeCsv(filters.cabildeoUris, lifted.cabildeo)
  }
  if (lifted.area?.length) {
    // Policy areas share the upstream `tag` param.
    filters.tag = mergeSpace(filters.tag, lifted.area.join(' '))
  }
