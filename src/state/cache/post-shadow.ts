...
function mergeShadow(
  ...
  return castAsShadow({
    ...
    viewer: {
      ...(post.viewer || {}),
      /*
       * `shadow.likeUri`/`repostUri`/`voteUri` may hold a non-uri optimistic
       * sentinel (e.g. `'pending'`), not just a real at-uri, so this can't be
       * branded at the `PostShadow` level - cast at the point it flows into
       * the branded `viewer` slot instead.
       */
