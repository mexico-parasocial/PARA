...
/*
 * `isType`, `matches`, `parse`, and `safeParse` operate on the generated
 * lexicon schema objects from '#/lexicons'. That codegen attaches the
 * validation surface directly to each schema, so these helpers take the
...
export function isType<S extends TypedSchema>(
   ...
   * def-schema check and break `$type`-discriminated unions. A present,
   * matching `$type` is therefore required. The nullish/object guard also
   * returns false (not throwing) for null/undefined input.
   */
  return (
    value != null &&
    typeof value === 'object' &&
