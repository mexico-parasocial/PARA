...
describe(`AdvancedSearchDialog serialize/parse`, () => {
  ...
  it(`round-trips PARA-specific filters unchanged`, () => {
    const filters: SearchFilters = {
      state: 'Jalisco',
      districtKey: 'iztapalapa-1',
      cabildeoPhase: 'voting',
      postType: 'raq',
      flairs: '||#ChequesEscolares,||#SalarioMinimo',
      party: 'Morena',
      verifiedPublicFigure: 'true',
      communityUris: 'at://alice/community,at://bob/community',
      cabildeoUris: 'at://alice/cabildeo',
      politicalCompassPositions: 'center-left,lib-left',
    }

    const state = parseAdvancedSearch('hello', filters)
    const out = serializeAdvancedSearch({
      query: state.query,
      exactPhrase: state.exactPhrase,
      negatedWords: state.negatedWords,
      ...
      filters: state.filters,
      state: state.state,
      districtKey: state.districtKey,
      cabildeoPhase: state.cabildeoPhase,
      postType: state.postType,
      flairs: state.flairs,
      party: state.party,
      verifiedPublicFigure: state.verifiedPublicFigure,
    })
  ...
  it(`creates PARA entity filter rows from sibling params on parse`, () => {
    const state = parseAdvancedSearch('', {
      communityUris: 'at://alice/community,at://bob/community',
      cabildeoUris: 'at://alice/cabildeo',
      politicalCompassPositions: 'center-left,lib-left',
    ...
    expect(state.filters.find(f => f.field === 'communities')?.value).toBe(
      'at://alice/community,at://bob/community',
    )
    expect(state.filters.find(f => f.field === 'cabildeos')?.value).toBe(
      'at://alice/cabildeo',
    )
    expect(state.filters.find(f => f.field === 'compassPositions')?.value).toBe(
      'center-left,lib-left',
    )
  })

  it(`serializes PARA entity filter rows as comma-separated sibling params`, () => {
    const out = serializeAdvancedSearch({
      ...emptySerializeState,
      filters: [
        makeFilter('communities', 'at://alice/community,at://bob/community'),
        makeFilter('cabildeos', 'at://alice/cabildeo'),
        makeFilter('compassPositions', 'center-left,lib-left'),
      ],
    })
    expect(out.filters.communityUris).toBe(
      'at://alice/community,at://bob/community',
    )
    expect(out.filters.cabildeoUris).toBe('at://alice/cabildeo')
    expect(out.filters.politicalCompassPositions).toBe('center-left,lib-left')
  })

  it(`lifts PARA entity operators from the query into filter rows`, () => {
    const state = parseAdvancedSearch(
      'presupuesto community:at://alice/community cabildeo:at://alice/cabildeo compass:auth-left area:education area:healthcare',
      {},
    )

    expect(state.query).toBe('presupuesto')
    expect(state.filters.find(f => f.field === 'communities')?.value).toBe(
      'at://alice/community',
    )
    expect(state.filters.find(f => f.field === 'cabildeos')?.value).toBe(
    ...
    expect(state.filters.find(f => f.field === 'policyAreas')?.value).toBe(
      'education,healthcare',
    )
  })

  it(`lifts PARA scalar operators from the query into dialog fields`, () => {
    const state = parseAdvancedSearch(
      'seguridad state:Jalisco district:iztapalapa-1 phase:voting party:Morena',
      {},
    )

    expect(state.query).toBe('seguridad')
    expect(state.state).toBe('Jalisco')
    expect(state.districtKey).toBe('iztapalapa-1')
    expect(state.cabildeoPhase).toBe('voting')
    expect(state.party).toBe('Morena')
  })

  it(`merges lifted PARA operators with existing sibling params`, () => {
    const state = parseAdvancedSearch('chat community:at://bob/community', {
      communityUris: 'at://alice/community',
    })
