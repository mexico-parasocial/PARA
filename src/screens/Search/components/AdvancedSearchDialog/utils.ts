...
/** PARA entity fields that are stored as comma-separated URIs/IDs. */
export const PARA_ENTITY_FIELDS = new Set<FilterField>([
  'communities',
  'cabildeos',
  'compassPositions',
  'policyAreas',
...
> = {
  include: {
    ...
    urls: 'url',
    tags: 'tag',
    communities: 'communityUris',
    cabildeos: 'cabildeoUris',
    compassPositions: 'politicalCompassPositions',
    policyAreas: 'tag',
  },
  exclude: {
    ...
    urls: 'excludeUrl',
    tags: 'excludeTag',
    // PARA search has no exclude variants yet; exclude rows map to the same
    // include param so the UI can still render them without losing values.
    communities: 'communityUris',
    cabildeos: 'cabildeoUris',
    compassPositions: 'politicalCompassPositions',
    policyAreas: 'tag',
...
/**
 * Joins two comma-separated value lists, dropping empties and duplicates while
 * preserving order. Used for PARA entity fields (communities, cabildeos, compass
 * positions, policy areas) whose route params are comma-separated URIs/IDs.
 */
...
export function parseAdvancedSearch(
  ...
  const includeValues: Record<FilterField, string> = {
    ...
    communities: mergeCsvValues(filters.communityUris, lifted.community),
    cabildeos: mergeCsvValues(filters.cabildeoUris, lifted.cabildeo),
    compassPositions: mergeCsvValues(
      filters.politicalCompassPositions,
      lifted.compass,
    ),
    // Policy areas are lifted via the `area:` operator; the upstream `tag`
    // param is intentionally surfaced as the free-text tags field instead.
    policyAreas: mergeCsvValues(undefined, lifted.area?.join(',')),
  }

  const filterRows: AdvancedFilter[] = []
  const MODES: FilterMode[] = ['include', 'exclude']
  for (const field of FILTER_FIELDS) {
    const isParaEntity = PARA_ENTITY_FIELDS.has(field)
    // PARA entity fields only support include mode for now.
    const modes = isParaEntity ? (['include'] as const) : MODES
  ...
  return {
    ...
    filters: filterRows,
    // Prefer operators lifted from the query text, falling back to the
    // structured filter params when present.
    state: lifted.state ?? filters.state,
    districtKey: lifted.district ?? filters.districtKey,
    cabildeoPhase: lifted.phase ?? filters.cabildeoPhase,
    postType: filters.postType,
    flairs: filters.flairs,
    party: lifted.party ?? filters.party,
    verifiedPublicFigure: filters.verifiedPublicFigure,
  }
}
...
export function serializeAdvancedSearch(state: {
  ...
  for (const filter of state.filters) {
    ...
    const delimiter = isParaEntity ? ',' : WHITESPACE_RE
  ...
  for (const [key, set] of valuesByKey) {
    // PARA entity route params are comma-separated URIs/IDs.
    const delimiter =
      key === 'communityUris' ||
      key === 'cabildeoUris' ||
      key === 'politicalCompassPositions'
        ? ','
        : ' '
    filters[key] = [...set].join(delimiter)
  }
  ...
  if (state.districtKey) filters.districtKey = state.districtKey
  if (state.cabildeoPhase) filters.cabildeoPhase = state.cabildeoPhase
  if (state.postType) filters.postType = state.postType
  if (state.flairs) filters.flairs = state.flairs
  if (state.party) filters.party = state.party
  if (state.verifiedPublicFigure)
    filters.verifiedPublicFigure = state.verifiedPublicFigure
  /*
