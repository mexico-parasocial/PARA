...
/**
 * The votes stat for the expanded anchor post. When the viewer follows some
 * of the post's recent voters, renders social proof - a face pile plus
 * "Voted for by A, B, and N others" - in place of the plain "N votes" text,
 * which it falls back to otherwise.
 *
 * Known voters are sourced client-side from a single `getLikes` request (100
 * likes, the API max per page), so they are a sample of the most recent
 * voters, not an exhaustive list. Only the faces and names are affected by
 * sampling - the "N others" count is derived from the post's total vote
 * count.
 */
export function LikesStat({post}: {post: AppBskyFeedDefs.PostView}) {
  ...
  if (!showKnownLikers) {
    return (
      <Link
        to={likesHref}
        label={l`Votes on this post`}
        onPress={onPressLikedBy}>
        <Text
          testID="votesCount-expanded"
          style={[a.text_md, t.atoms.text_contrast_medium]}>
          <Trans comment="Vote count display, the <0> tags enclose the number of votes in bold (will never be 0)">
            <Text style={[a.text_md, a.font_semi_bold, t.atoms.text]}>
              {formatPostStatCount(likeCount)}
            </Text>{' '}
            <Plural value={likeCount} one="vote" other="votes" />
          </Trans>
        </Text>
      </Link>
    )
  ...
  const rowLabel =
    names.length >= 2
      ? others > 0
        ? l`${names[0].displayName}, ${names[1].displayName}, and ${othersLabel} voted for this`
        : l`${names[0].displayName} and ${names[1].displayName} voted for this`
      : others > 0
        ? l`${names[0].displayName} and ${othersLabel} voted for this`
        : l`${names[0].displayName} voted for this`
   ...
   * the votes list. Same pattern as NotificationFeedItem's author links.
   */
    ...
    <View style={[a.w_full, a.flex_row]}>
      <Link
        ...
        <Text
          ...
          {names.length >= 2 ? (
            others > 0 ? (
              <Trans comment="Social proof on the votes stat; the bolded names are people the viewer follows who voted for the post, and the count is the remaining number of votes">
              ...
              <Trans comment="Social proof on the votes stat; the bolded names are people the viewer follows who voted for the post and are its only votes">
                {nameLink(names[0])} and {nameLink(names[1])} voted for this
              </Trans>
            )
          ) : others > 0 ? (
            <Trans comment="Social proof on the votes stat; the bolded name is a person the viewer follows who voted for the post, and the count is the remaining number of votes">
            ...
            <Trans comment="Social proof on the votes stat; the bolded name is a person the viewer follows who voted for the post and is its only vote">
              {nameLink(names[0])} voted for this
            </Trans>
          )}
        </Text>
      </Link>
